  ...
  return (
    <section ref={ref} className="section-dark relative py-32 overflow-hidden">
      ...
      <motion.div className="absolute top-[40%] left-[45%] w-[280px] h-[280px] rounded-full pointer-events-none"
        style={{ background: "radial-gradient(circle, rgba(34,211,238,0.22) 0%, transparent 70%)", filter: "blur(50px)" }}
        ...
        <motion.div initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="grid sm:grid-cols-2 gap-4 max-w-2xl mx-auto mb-10">
            <motion.div whileHover={{ y: -4 }} className="group relative rounded-2xl p-6 text-left overflow-hidden gradient-border"
              style={{ background: "linear-gradient(135deg, rgba(59,130,246,0.10), rgba(34,211,238,0.06))" }}>
              <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full opacity-30 group-hover:opacity-60 transition-opacity duration-700" style={{ background: "radial-gradient(circle,#3B82F6,transparent 70%)", filter: "blur(30px)" }} />
              <div className="relative z-10">
                ...
                <button className="magnetic-btn w-full px-5 py-3 rounded-xl text-[13px] font-semibold text-white flex items-center justify-center gap-2"
                  style={{ background: "linear-gradient(120deg,#3B82F6,#22D3EE)", boxShadow: "0 6px 24px rgba(59,130,246,0.4)" }}>
                  Start enrolling
            ...
            <motion.div whileHover={{ y: -4 }} className="group relative rounded-2xl p-6 text-left overflow-hidden gradient-border"
              style={{ background: "linear-gradient(135deg, rgba(139,92,246,0.10), rgba(76,29,149,0.06))" }}>
              <div className="absolute -top-12 -right-12 w-40 h-40 rounded-full opacity-30 group-hover:opacity-60 transition-opacity duration-700" style={{ background: "radial-gradient(circle,#8B5CF6,transparent 70%)", filter: "blur(30px)" }} />
              <div className="relative z-10">
                <div className="text-[11px] font-semibold text-[#A78BFA] tracking-wider uppercase mb-2">For Employers · HR</div>
                ...
                <button className="magnetic-btn w-full px-5 py-3 rounded-xl text-[13px] font-semibold text-white flex items-center justify-center gap-2"
                  style={{ background: "linear-gradient(120deg,#8B5CF6,#6D28D9)", boxShadow: "0 6px 24px rgba(139,92,246,0.4)" }}>
                  Talk to sales
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
                </button>
              </div>
        ...
        <div className="mt-24 pt-8 border-t border-white/[0.06]">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-md flex items-center justify-center shadow-glow-blue" style={{ background: "linear-gradient(135deg,#3B82F6,#22D3EE,#8B5CF6)" }}>
