  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <motion.div
        ...
        style={{ background: "radial-gradient(circle, rgba(37,99,235,0.45), transparent 65%)", filter: "blur(60px)" }}
        animate={{ x: [0, 80, -40, 0], y: [0, 60, -30, 0] }}
        transition={{ duration: 22, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-[10%] right-[-10%] w-[640px] h-[640px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(34,211,238,0.35), transparent 65%)", filter: "blur(60px)" }}
        animate={{ x: [0, -60, 40, 0], y: [0, 50, -40, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-[-20%] left-[20%] w-[820px] h-[820px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(139,92,246,0.40), transparent 65%)", filter: "blur(70px)" }}
        animate={{ x: [0, 50, -50, 0], y: [0, -40, 30, 0] }}
        transition={{ duration: 30, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-[40%] left-[55%] w-[420px] h-[420px] rounded-full"
        style={{ background: "radial-gradient(circle, rgba(76,29,149,0.45), transparent 65%)", filter: "blur(60px)" }}
      ...
      {/* Vignette */}
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 80% 60% at 50% 40%, transparent 0%, rgba(5,8,22,0.6) 100%)" }} />
    </div>
  );
}
...
function HeroDashboardMockup() {
  return (
    <div className="w-full overflow-hidden" style={{ background: "linear-gradient(180deg, #0A0E27 0%, #0F1531 100%)", minHeight: 460 }}>
      <div className="flex h-full" style={{ minHeight: 460 }}>
        {/* Sidebar */}
        <div className="w-[210px] shrink-0 border-r border-white/[0.05] p-4 flex flex-col gap-1" style={{ background: "rgba(10,14,39,0.6)" }}>
          <div className="flex items-center gap-2 mb-6 px-2 pt-1">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-[#3B82F6] via-[#22D3EE] to-[#8B5CF6] flex items-center justify-center shadow-glow-blue">
          ...
          {[
            ...
            <div key={item.label} className={`flex items-center gap-2.5 px-2.5 py-2 rounded-lg cursor-pointer transition-all ${item.active ? "text-white" : "text-white/40 hover:bg-white/[0.04]"}`} style={item.active ? { background: "linear-gradient(90deg, rgba(59,130,246,0.18), rgba(139,92,246,0.12))", boxShadow: "inset 0 0 0 1px rgba(59,130,246,0.25)" } : {}}>
        ...
        <div className="flex-1 p-5 overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="rounded-xl p-3.5 border border-white/[0.06]" style={{ background: "rgba(255,255,255,0.025)" }}>
              ...
              <div className="w-full h-1 bg-white/[0.06] rounded-full mt-2">
                <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg,#3B82F6,#22D3EE,#8B5CF6)" }} initial={{ width: 0 }} animate={{ width: "80%" }} transition={{ duration: 1.6, delay: 1.8, ease: [0.16,1,0.3,1] }} />
              </div>
            </div>
            <div className="rounded-xl p-3.5 border border-white/[0.06]" style={{ background: "rgba(255,255,255,0.025)" }}>
          ...
          <div className="rounded-xl p-4 border border-white/[0.06] mb-3" style={{ background: "rgba(255,255,255,0.025)" }}>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[9px] text-white/40 px-2 py-0.5 rounded-full" style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.18)" }}>1Y</span>
            </div>
            <svg viewBox="0 0 300 80" className="w-full" style={{ height: 70 }}>
              ...
              <motion.circle cx="300" cy="5" r="4" fill="#22D3EE" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 3.6, type: "spring", stiffness: 300 }} style={{ filter: "drop-shadow(0 0 8px #22D3EE)" }} />
            </svg>
          </div>
    ...
    <section ref={ref} className="section-dark relative min-h-[115vh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div style={{ y: contentY, opacity: contentOpacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-32">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
            ...
            style={{ background: "linear-gradient(120deg, #3B82F6 0%, #22D3EE 35%, #8B5CF6 70%, #6D28D9 100%)", boxShadow: "0 4px 24px rgba(59,130,246,0.45), 0 0 0 1px rgba(255,255,255,0.1) inset" }}>
            ...
            <span className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
              style={{ background: "linear-gradient(120deg, #22D3EE, #8B5CF6, #3B82F6)" }} />
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 1 }}
          ...
          <div className="flex items-center gap-2">
            <div className="flex -space-x-2">
              {[1,2,3,4].map((i) => (
                <div key={i} className="w-7 h-7 rounded-full border-2 border-[#050816]" style={{ background: `linear-gradient(135deg, hsl(${i*60+200},80%,60%), hsl(${i*60+260},70%,55%))` }} />
              ))}
            </div>
      ...
      <motion.div style={{ y: mockupY, scale: mockupScale }} className="relative z-10 mt-20 w-full max-w-[1040px] mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 80 }} animate={{ opacity: 1, y: 0 }}
          ...
          {/* Glow behind */}
          <div className="absolute -inset-8 rounded-3xl opacity-60" style={{ background: "radial-gradient(ellipse 60% 80% at 50% 50%, rgba(59,130,246,0.35), rgba(139,92,246,0.25) 40%, transparent 70%)", filter: "blur(40px)" }} />
