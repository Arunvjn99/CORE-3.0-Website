...
const badges = [
  { label: "SOC 2 Type II", desc: "Independently audited security controls", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg>, gradient: "from-[#FF9500] to-[#D67E00]" },
  { label: "ERISA Compliant", desc: "Full DOL & IRS regulatory adherence", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>, gradient: "from-[#FFB347] to-[#FF9500]" },
  { label: "256-bit Encryption", desc: "Bank-level data protection, end-to-end", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>, gradient: "from-[#D67E00] to-[#B36900]" },
  { label: "WCAG 2.1 AA", desc: "Accessible to every employee, every ability", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>, gradient: "from-[#10B981] to-[#FFB347]" },
  { label: "Multi-Language", desc: "English, Spanish, French, Hindi, Mandarin", icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>, gradient: "from-[#D67E00] to-[#F43F5E]" },
  ...
  return (
    <section ref={ref} className="section-dark relative py-32 overflow-hidden">
      <div className="absolute inset-0" style={{
        background: "radial-gradient(ellipse 60% 40% at 50% 50%, rgba(255,149,0,0.10) 0%, transparent 60%)"
      }} />
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-5" style={{ background: "rgba(255,149,0,0.08)", border: "1px solid rgba(255,149,0,0.22)" }}>
            <div className="w-1.5 h-1.5 rounded-full bg-[#FF9500]" style={{ boxShadow: "0 0 8px #FF9500" }} />
          ...
          {badges.map((b, i) => (
            <motion.div key={b.label} initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              <div className={`w-11 h-11 rounded-lg bg-gradient-to-br ${b.gradient} p-[1px] shrink-0`}>
                <div className="w-full h-full rounded-lg flex items-center justify-center text-white/70 group-hover:text-white transition-colors" style={{ background: "#0A0F1A" }}>
                  {b.icon}
                </div>
              </div>
              <div>
